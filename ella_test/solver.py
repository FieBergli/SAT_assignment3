...
def dpll(clauses, assignment, num_vars, counters):
    ...
    for try_val in (pref_val, not pref_val):
      ...
      sat, final_assignment, counters = dpll(new_clauses, new_assignment, num_vars, counters)
      if not sat: 
...
def solve_cnf_dlcs(clauses, num_vars):
    ...
    print(f"Splits DLCS: {counters['splits']}")
    print(f"Backtracks DLCS: {counters['backtracks']}")
    print(f"Recursive calls DLCS: {counters['calls']}")
