...
def solve_cnf_dlcs(clauses, num_vars, log=False):
    """
    Solve a CNF formula using the Dynamic Largest Combined Sum (DLCS) heuristic.
    Return:
    ...
    string += f"Runtime DLCS: {runtime}\n"
    string += f"Splits DLCS: {counters['splits']}\n"
    string += f"Backtracks DLCS: {counters['backtracks']}\n"
    string += f"Recursive calls DLCS: {counters['calls']}\n"
