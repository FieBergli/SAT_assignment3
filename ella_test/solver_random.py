...
def solve_cnf_random(clauses, num_vars, log=False):
    """
    Solve a CNF formula using a random branching heuristic.
    Return:
    ...
    string += f"Runtime RANDOM: {runtime}\n"
    string += f"Splits RANDOM: {counters['splits']}\n"
    string += f"Backtracks RANDOM: {counters['backtracks']}\n"
    string += f"Recursive calls RANDOM: {counters['calls']}\n"
